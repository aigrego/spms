...
/* POST /api/auth/login { username, password } → session cookie + the user.
   Lark-only accounts carry '!lark' as their hash — never a valid bcrypt hash,
   so verifyPassword fails and password login stays disabled for them. */
export const POST = route(async (req) => {
  ...
  await ensureCurrentMember(u); // lazy (user → member) projection on first login
  const c = await createSessionCookie(u);
  const res = ok({ id: u.id, username: u.username, name: u.name, role: u.role });
