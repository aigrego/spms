...
/* Resolve a requirement display key ("FR-12") → its internal uuid.
   Returns null for an empty key; undefined when provided but not found. */
...
/* Resolve an issue by its display key → the internal row. */
async function findByKey(key: string) {
  return db.query.issues.findFirst({ where: eq(issues.key, key) });
}
...
/* Load any member (human or agent) by id. */
async function loadMember(memberId: string | null | undefined) {
...
async function teamForProject(projectId: string | null) {
...
async function resolveSprintProject(sprintId: string, projectId: string | null): Promise<string | null> {
  const [sp] = await db
...
export async function listIssues(filter?: { team?: string; assignee?: string; project?: string }) {
  const conds = [];
...
export async function createIssue(actor: Actor, input: CreateIssueInput) {
  ...
  const assigneeMember = await loadMember(input.assigneeId);
  ...
  if (input.sprintId) {
    resolvedProjectId = await resolveSprintProject(input.sprintId, resolvedProjectId);
  }

  // Legacy team inherited from the project (team is retired from the UI).
  const teamId = await teamForProject(resolvedProjectId);

  // Allocate the next per-type display key (BUG-/TKT-/BLG-) + insert.
  const issueType = input.type ?? 'ticket';
  const id = crypto.randomUUID();
  const key = await nextKey(TYPE_PREFIX[issueType]);
  ...
  if (input.labels?.length) {
    await db
      .insert(issueLabels)
      .values(input.labels.map((labelId) => ({ issueId: id, labelId })))
      .onConflictDoNothing();
  }
  ...
  if (agent) await onAgentAssigned(id, agent);
...
export async function updateIssue(actor: Actor, key: string, input: UpdateIssueInput) {
  ...
  if (nextSprintId) {
    ...
    patch.projectId = await resolveSprintProject(nextSprintId, effProject ?? null);
  }
  // Keep the legacy teamId aligned with the issue's (possibly changed) project.
  if (patch.projectId !== undefined) {
    patch.teamId = await teamForProject(patch.projectId ?? null);
  }
  ...
  if (input.assigneeId !== undefined) {
    ...
    const m = await loadMember(input.assigneeId);
  ...
  if (input.labels !== undefined) {
    ...
    if (input.labels.length) {
      await db
        .insert(issueLabels)
        .values(input.labels.map((labelId) => ({ issueId: existing.id, labelId })))
        .onConflictDoNothing();
    }
  }

  if (newAgent) {
    await onAgentAssigned(existing.id, newAgent);
...
export async function toggleSubIssue(key: string, subId: string, status: IssueStatus) {
  void key;
  const sub = await db.query.subIssues.findFirst({ where: eq(subIssues.id, subId) });
